...
export function UserRoleTable({
  ...
  const updateUser = async (userId: string) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(payload?.error ?? 'Unable to update user settings.');
      }

      refresh();
    } catch (submitError) {
      setError(submitError instanceof Error ? submitError.message : 'Unable to update user settings.');
    } finally {
      setPendingKey(null);
    }
  };
  ...
  const applyBulkUpdate = async () => {
    ...
    try {
      ...
      const payload = (await response.json().catch(() => null)) as { error?: string } | null;
      if (!response.ok) {
        throw new Error(payload?.error ?? 'Unable to apply bulk user action.');
      }
      ...
      setError(submitError instanceof Error ? submitError.message : 'Unable to apply bulk user action.');
    } finally {
      setPendingKey(null);
    }
  };
  ...
  const applyBulkSessionReset = async () => {
    ...
    try {
      ...
      const payload = (await response.json().catch(() => null)) as { error?: string } | null;
      if (!response.ok) {
        throw new Error(payload?.error ?? 'Unable to revoke selected sessions.');
      }

      setSelectedIds([]);
      refresh();
    } catch (submitError) {
      setError(submitError instanceof Error ? submitError.message : 'Unable to revoke selected sessions.');
    } finally {
      setPendingKey(null);
    }
  };
    ...
    <div className="rounded-3xl border border-border bg-card p-6 shadow-xl">
      ...
      <div className="space-y-3">
        {users.map((user) => {
          ...
          return (
            <div key={user.id} className="grid gap-4 rounded-2xl border border-border/70 bg-background/60 p-4 xl:grid-cols-[auto_1.1fr_0.9fr_1.1fr_auto] xl:items-center">
              <div className="pt-1">
