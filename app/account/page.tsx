      ...
      <main className="mx-auto max-w-6xl px-4 py-8 sm:px-6">
        <section className="rounded-3xl border border-border bg-card p-6 shadow-xl sm:p-8">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <p className="text-xs font-medium uppercase tracking-[0.2em] text-teal-accent">Личный кабинет</p>
              <h1 className="mt-3 text-3xl font-semibold text-foreground">{overview.name}</h1>
            ...
            <div className="flex flex-wrap gap-3">
              <Button asChild className="bg-teal-dark text-white hover:bg-teal-medium dark:bg-teal-accent dark:text-[#09090B] dark:hover:bg-seafoam">
          ...
          <div className="mt-6 grid gap-4 md:grid-cols-3">
            <div className="rounded-2xl border border-border/70 bg-background/60 p-4">
              ...
              <p className="mt-2 text-lg font-semibold text-foreground">{overview.sellerProfile?.name ?? 'Не создан'}</p>
            </div>
            <div className="rounded-2xl border border-border/70 bg-background/60 p-4">
              <p className="text-sm text-muted-foreground">Объявлений о продаже</p>
              <p className="mt-2 text-lg font-semibold text-foreground">{overview.saleListings.length}</p>
            </div>
            <div className="rounded-2xl border border-border/70 bg-background/60 p-4">
              <p className="text-sm text-muted-foreground">Запросов в подбор</p>
              <p className="mt-2 text-lg font-semibold text-foreground">{overview.wantedListings.length}</p>
            </div>
          </div>
        </section>
                ...
                overview.saleListings.map((listing) => {
                  ...
                  return (
                    <Link
                      ...
                      className="grid gap-4 rounded-2xl border border-border/70 bg-background/60 p-4 transition-colors hover:border-teal-accent/35 hover:bg-background/80 md:grid-cols-[160px_1fr]"
                    >
                      <div className="overflow-hidden rounded-xl border border-border/70 bg-muted/20">
                        {cover ? (
                          <img src={cover} alt={`${listing.make} ${listing.model}`} className="h-32 w-full object-cover" />
                        ) : (
