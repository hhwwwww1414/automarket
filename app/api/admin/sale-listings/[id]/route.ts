...
export async function PATCH(
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : 'Unable to update sale listing.';
...
export async function DELETE(
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : 'Unable to delete sale listing.';
    return NextResponse.json({ error: message }, { status: 400 });
  }
}
