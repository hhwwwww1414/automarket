...
export async function PATCH(
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : 'Unable to update user settings.';
...
export async function POST(
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : 'Unable to revoke user sessions.';
    return NextResponse.json({ error: message }, { status: 400 });
  }
}
