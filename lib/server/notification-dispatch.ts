...
async function sendEmailNotification(input: {
  ...
  const hrefLine = input.href ? `<p><a href="${input.href}">Open notification</a></p>` : '';

  await emailTransporter.sendMail({
    from: serverEnv.smtpFrom,
    to: input.email,
    subject: input.title,
    text: [input.title, '', input.message, input.href ? `Open: ${input.href}` : ''].filter(Boolean).join('\n'),
  ...
  if (notification.user.telegramNotificationsEnabled && notification.user.telegramChatId) {
    tasks.push(
      sendTelegramMessage(
        ...
        [notification.title, notification.message, absoluteHref ? `Open: ${absoluteHref}` : ''].filter(Boolean).join('\n')
      )
    );
